...
uart_com::uart_com(u32 ULITE_DEVICE_ID,u32 ULITE_INT_IRQ_ID,XUartLite* UPTR,XIntc* InterruptController,u32 XINTC_DEV_ID,XUartLite_Handler FuncPtr){
	...
	status=XIntc_Initialize(InterruptController, XINTC_DEV_ID);

	status=XIntc_Connect(InterruptController, ULITE_INT_IRQ_ID,(XInterruptHandler)XUartLite_InterruptHandler,(void *)UPTR);
...
void uart_com::uart_com_send(u8* Send_Array,int length)
{
	...
	for(int j=0;j<(length/16);j++)
	{
		...
		if(j==(length/16)-1)
		{
			XUartLite_Send(UPTR, (Send_Array+(16*j)), 16-rem_byte);
		}
		else
		{
